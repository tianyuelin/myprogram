...
public class Xtgl {
	public static void main(String[] args) {
		...
		/*
		 * Entry:嵌套接口（静态的接口）
		 * 将键值对的对应关系封装成了对象，即键值对的对象。
		 * map对象的entrySet方法可以得到一个set集合，里面装的是Entry接口类型，即将映射关系装入set集合
		 * 实现步骤：
		 * 1.调用map集合中的entrySet方法，即将集合中的映射关系对象存储到set集合中
		 * 2.迭代set集合
		 * 3.获取set集合的元素，是映射关系对象
		 * 4.通过映射关系对象的方法，getKey和geValue，获取键值对
		 * */
		//1.调用map集合中的entrySet方法，即将集合中的映射关系对象存储到set集合中
